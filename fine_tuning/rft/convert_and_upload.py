...
try:
    grader_yaml_file = os.path.join(script_dir, 'data/grader.yaml')
    grader_json_file = os.path.join(output_dir, f'{current_date}-rft-grader.json')
    ...
    print(f"[ERROR] grader 변환 실패: {e}")

################ train_data ###############
train_file = os.path.join(output_dir, f'{current_date}-rft-train.jsonl')
...
################ val_data ###############
val_file = os.path.join(output_dir, f'{current_date}-rft-val.jsonl')
